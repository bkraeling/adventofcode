...
const processDigits = (digits) => {
  ...
  // Find 6
  const six = remainingDigits.find((rd) => common(rd, five).length === 5);
...
fileStream
  ...
  .on("end", function () {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      const translatedOutput = outputs.map((o) => digitsMap[o]);
      for (let j = 0; j < translatedOutput.length; j++) {
        switch (translatedOutput[j]) {
          case 1:
          case 4:
          case 7:
          case 8:
