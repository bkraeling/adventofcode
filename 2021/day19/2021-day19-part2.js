  ...
  .on("end", function () {
    ...
    for (let i = 0; i < data.length; i++) {
      for (let j = 0; j < data.length; j++) {
        const tree = new BinaryTree(arrayize(`[${data[i]},${data[j]}]`), 0);
        ...
        console.log(tree.toString());
        const treeMagnitude = tree.getMagnitude();
        console.log({ treeMagnitude });
        if (treeMagnitude > highestMagnitude) {
          highestMagnitude = treeMagnitude;
        }
      }
    }
