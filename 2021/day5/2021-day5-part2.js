  ...
  .on("end", function () {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (beginX === endX) {
        ...
        let yValues;
        if (endX > beginX) {
          xValues = _.range(beginX, endX + 1);
        } else {
          xValues = _.range(endX, beginX + 1).reverse();
        }
        if (endY > beginY) {
          yValues = _.range(beginY, endY + 1);
        } else {
          yValues = _.range(endY, beginY + 1).reverse();
        }
        // console.log(xValues);
        // console.log(yValues);

        for (let j = 0; j < xValues.length; j++) {
          linePoints.push(`${xValues[j]},${yValues[j]}`);
        }
        // console.log(linePoints);
      }
