...
const NECESSARY_UNUSED = 30000000;
...
fileStream
  ...
  .on("end", function () {
    ...
    const TOTAL_UNUSED = TOTAL_DISK - root.getSize();
    const TO_FREE_UP = NECESSARY_UNUSED - TOTAL_UNUSED;
    console.log({
      TOTAL_DISK,
    ...
    while (allFiles.length > 0) {
      ...
      if (
        file.isDirectory() &&
        size > TO_FREE_UP &&
        size < bestDirectory.getSize()
      ) {
        bestDirectory = file;
      }
    }
    console.log(
      `The best directory is ${bestDirectory.getName()}, which frees up ${bestDirectory.getSize()} of space`
    );
  })
  .read();
